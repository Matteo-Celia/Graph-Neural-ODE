...
def broadcast_receiver_nodes_to_edges(graph: GraphsTuple):
    return torch.from_numpy(graph.nodes).index_select(index=torch.from_numpy(graph.receivers).long(), dim=0)


def broadcast_sender_nodes_to_edges(graph: GraphsTuple):
    return torch.from_numpy(graph.nodes).index_select(index=torch.from_numpy(graph.senders).long(), dim=0)
...
class EdgeBlock(nn.Module):
    ...
    def forward(self, graph: GraphsTuple):
        ...
        if self._use_edges:
            edges_to_collect.append(graph.edges)  # edge feature  (50,6)
        ...
        collected_edges = torch.cat(torch.tensor(edges_to_collect), dim=1)
